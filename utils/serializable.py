        ...
        if(mod_name in sys.modules.keys()):
            ...
            for key in new_obj.__dict__.keys():
                if(key in json_dict.keys()):
                    ...
                    if(current_value == None or type(current_value) in AUTHORIZED_TYPE):
...
def __deserialize_list(json_list):
    ...
    for item in json_list:
        if(item == None or type(item) in AUTHORIZED_TYPE):
...
def __deserialize_dict(json_dict):
    ...
    for key in json_dict.keys():
        ...
        if(json_value == None or type(json_value) in AUTHORIZED_TYPE):
