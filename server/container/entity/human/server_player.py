...
class ServerPlayer:
    ...
    def get_mouvement_with_timestep(self, timestep):
        return self.player.velocity * timestep / (1_000_000_000 / self.__server_tps)
        ...
        if action['type'] == "key_action":
            if action['action'] == key_action.ACTION_DOWN:
                if action['key'] == key_action.KEY_RIGHT:
                    ...
                    self.player.position += self.get_mouvement_with_timestep(self.clock.get_time() - data['timestamp'])
    ...
    def update_player(self, world_size):
        ...
        self.player.position += self.get_mouvement_with_timestep(self.clock.time_step())
